...
public class GraphExtractor {
	...
	private ModuleGraph processAtomic(Node stmt) {
		...
		if (latticeType.equalsIgnoreCase("rd")) {
			// return the graph as is,ignore atomic
			return seqGraph;
		} else {
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (child.getNodeType() == Document.ELEMENT_NODE) {
				if (child.getNodeName().equals("OPTION")) {
					...
					doGraph.addEdge(
							new ModuleEdge(start, g.getStartNode(), startID, CounterHandler.getNoChange(messageSet)));
					doGraph.addEdge(new ModuleEdge(g.getEndNode(), end, endID, CounterHandler.getNoChange(messageSet)));
	...
	// in backward the transfer functions are ID for conditionals
